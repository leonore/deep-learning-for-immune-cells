...
Neural networks are only limited by logic and can be constructed for regression tasks such that the model is trained to map from data, e.g. images, to real-values from a continuous range. In the context of imaging data, this could be used to extract numerical features from an image. Xie et al. and Xue and Ray show promising results for using convolutional neural networks to extract numerical features from images of cell by using neural networks as a regression model that counts the number of cells in an image (\cite{https://arxiv.org/pdf/1708.03307.pdf, https://www.robots.ox.ac.uk/~vgg/publications/2015/Xie15/weidi15.pdf}).
